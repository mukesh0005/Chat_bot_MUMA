...
def on_click_callback(): #called on click of submit button
    ...
    with get_openai_callback() as cb:
        ...
        similar_docs = vectorstore.similarity_search(
            ...
            k=5  # return relevant docs
        )
        
        # create a prompt with the human prompt and the context from the most similar documents
        prompt = f"""
            /You are a helpful chatbot named Bullbot that interact with students with queries./
            /Ask the students for their UID before solving their queries. Ask for their UID using statements like 'Before I can assist you with your query, could you please provide your UID?'/
            /You don't need to verify if the UID is accurate./
            /If you are provided tasks that can't be done by you such as sending mails, state that a request has been placed for this to the relevant department./
...
load_css()  # load the css to allow for styles.css to affect look and feel
initialize_session_state()  # initialize the history buffer that is stored in UI

# create the Streamlit UI
st.markdown("<h1 style='text-align: center;'>🤘🏻 USF BullBot 🤘🏻</h1>", unsafe_allow_html=True)

# Create the Streamlit UI
st.markdown("<h1 style='text-align: center;'>🤘🏻 USF BullBot 🤘🏻</h1>", unsafe_allow_html=True)

# Initialize student_info in session_state if it doesn't exist
if 'student_info' not in st.session_state:
    st.session_state.student_info = StudentInfo(uid="", status="")

# Text box for user UID
st.session_state.student_info.uid = st.text_input("Enter your UID:", st.session_state.student_info.uid)

# Select box for Current or Past
st.session_state.student_info.status = st.selectbox("Are you a current or past student?", ["Current", "Past"], index=0 if st.session_state.student_info.status == "Current" else 1)
